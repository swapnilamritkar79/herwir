		...
		foreach($invoiceitems as $invoiceitem)
		{
           ...
           $mform->addElement('html','<tr>
			...
			$mform->addElement('html','<input type="number" min="1" class="price-textbox" name="quantity['.$invoiceitem->id.']" onchange="changequantity(this,'.$invoiceitem->id.')" value="'.$invoiceitem->quantity.'" onkeypress="return isNumberKey(event)">');
			  ...
			  <td>
                <div class="total">'.sprintf ("%.2f",($invoiceitem->price * $invoiceitem->quantity)*($CFG->tax/100)).'</div>
              </td>
			   <td>
                <div class="total">'.sprintf ("%.2f",($invoiceitem->price * $invoiceitem->quantity)+($invoiceitem->price * $invoiceitem->quantity*($CFG->tax/100))).'</div>
              </td>
